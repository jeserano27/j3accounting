...
type Supplier = { id: string; name: string; payment_terms: number };

function newLine(): BillLine {
  return { id: Math.random().toString(36).slice(2), description: '', quantity: '1', unit_price: '', tax_rate: '12' };
}

function lineAmount(l: BillLine) {
  try { return new Decimal(l.quantity || '0').mul(new Decimal(l.unit_price || '0')); }
  catch { return new Decimal(0); }
}

function lineTax(l: BillLine) {
  try { return lineAmount(l).mul(new Decimal(l.tax_rate || '0').div(100)); }
  catch { return new Decimal(0); }
}
...
export default function NewBillPage() {
  ...
  async function loadData() {
    ...
    const { data } = await supabase
      .from('suppliers').select('id,name,payment_terms').eq('company_id', uc.company_id).eq('is_active', true).order('name');
    setSuppliers(data ?? []);
  }
  ...
  function handleSupplierChange(id: string) {
    ...
    if (sup) {
      const d = new Date(billDate); d.setDate(d.getDate() + sup.payment_terms);
      setDueDate(d.toISOString().slice(0, 10));
    }
  }

  function updateLine(id: string, field: keyof BillLine, value: string) {
  ...
  async function handleSave(status: 'draft' | 'approved') {
    ...
    if (!billDate || !dueDate) { setError('Bill date and due date are required.'); return; }
    ...
    const { data: numData, error: numErr } = await supabase
      .rpc('next_bill_number', { p_company_id: companyId });
    ...
    if (billErr || !bill) { setError(billErr?.message ?? 'Failed to create bill.'); setSaving(false); return; }
    ...
    const { error: linesErr } = await supabase.from('bill_lines').insert(linePayload);
    if (linesErr) {
      await supabase.from('bills').delete().eq('id', bill.id);
      setError(linesErr.message); setSaving(false); return;
    }

    router.push(`/ap/${bill.id}`);
  }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-slate-500 text-sm mt-0.5">Record a supplier bill / purchase invoice</p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-xs font-medium text-slate-700 mb-1.5">Supplier *</label>
          ...
          <div>
            <label className="block text-xs font-medium text-slate-700 mb-1.5">Supplier Invoice Ref</label>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <label className="block text-xs font-medium text-slate-700 mb-1.5">Bill Date *</label>
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
          ...
          {lines.map(line => (
            <div key={line.id} className="grid grid-cols-12 gap-2 px-6 py-3 items-center">
              ...
              <div className="col-span-1">
                <input type="number" min="0" step="1" value={line.quantity} onChange={e => updateLine(line.id, 'quantity', e.target.value)}
              ...
              <div className="col-span-2">
                <input type="number" min="0" step="0.01" value={line.unit_price} onChange={e => updateLine(line.id, 'unit_price', e.target.value)}
                  placeholder="0.00"
                  className="w-full px-2 py-2 border border-slate-200 rounded-lg text-sm text-right focus:outline-none focus:ring-2 focus:ring-teal-500/30 focus:border-teal-400" />
              </div>
              <div className="col-span-1">
        ...
        <div className="px-6 py-4 bg-slate-50 border-t border-slate-200 space-y-2">
          <div className="flex justify-end gap-8 text-sm text-slate-600">
            <span>Subtotal</span><span className="font-mono w-32 text-right">{formatMoney(subtotal.toFixed(2))}</span>
          </div>
          <div className="flex justify-end gap-8 text-sm text-slate-600">
            <span>VAT</span><span className="font-mono w-32 text-right">{formatMoney(taxTotal.toFixed(2))}</span>
          </div>
          <div className="flex justify-end gap-8 text-base font-bold text-slate-900 pt-2 border-t border-slate-200">
            <span>Total</span><span className="font-mono w-32 text-right">{formatMoney(total.toFixed(2))}</span>
          </div>
        </div>
      </div>
